...
# Словарь терминов
dict = {
    "Распределение": "Закон, описывающий область значений случайной величины и соответствующие вероятности появления этих значений.",
...
keyboard_dict.add(
    telebot.types.InlineKeyboardButton(
        text="Распределение", callback_data="distribution"
    )
)
keyboard_dict.add(
    telebot.types.InlineKeyboardButton(text="Среднее", callback_data="mediana")
)
keyboard_dict.add(
...
def callback_query(call):
    if call.data == "example_bioequal":
        ...
        file = open("../../media/data/Пример_биоэквивал.xls", "rb")
...
def default_test(message):
    if message.text.lower() == "bioequal":
        ...
        keyboard01.add(
            telebot.types.InlineKeyboardButton(
                text="Пример файла", callback_data="example_lifeline"
            )
        )
        keyboard01.add(
            telebot.types.InlineKeyboardButton(

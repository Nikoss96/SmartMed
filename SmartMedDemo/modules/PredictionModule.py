...
class PredictionModule(Module, PredictionDashboard):
    ...
    def _init_settings(self, model_type: str) -> dict:
        ...
        self.model = ModelManipulator(
            x=self.df_X_train, y=self.df_Y_train, model_type=self.settings["model"]
        ).create()
        self.model.fit()
        ...
        settings["x"] = self.pp.df.columns.tolist() + [
            i for i in range(self.settings["bin"])
        ]
    ...
    def _prepare_dashboard_settings(self):
        ...
        if self.settings["model"] == "linreg":
            ...
            for i in range(count):
                for j in range(i, count):
                    ...
                    if i == j:
                        ...
                        data_name = str(
                            self.df_X.columns[i] + " * " + str(self.df_X.columns[j])
                        )
                    print(data_name)
            ...
            self.df_Y_test = dfY_test
            self.model = ModelManipulator(
                x=self.df_X_train, y=self.df_Y_train, model_type="polyreg"
            ).create()
            self.model.fit()
            self.mean = sum(dfY_test) / len(dfY_test)
            ...
            for name in init_unique_values:
                number_class.append(
                    self.df_Y[list(init_df[self.settings["variable"]]).index(name)]
                )
            ...
            for metric in self.settings.keys():
                if metric == "model":
                    ...
                    metric == "tree"
                    or metric == "table"
                    or metric == "indicators"
                    or metric == "distributions"
                    or metric == "prediction"
                ):
                    settings["metrics"].append(metric)
                elif self.settings[metric]:

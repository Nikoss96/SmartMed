...
class ComparativeDashboard(Dashboard):
    def _generate_layout(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H1(children="Сравнительный анализ"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    html.H3(children=self.settings["type"]),
                    style={"text-align": "center"},
                ),
    ...
    def _generate_test_kolmagorova_smirnova(self):
        ...
        cont_columns = [
            v for v in columns_list if v not in set(cat_columns) & set(columns_list)
        ]
        ...
        self.app.callback(
            dash.dependencies.Output("kolm_smirn_table", "data"),
            dash.dependencies.Output("kolm_smirn_table", "columns"),
            dash.dependencies.Input("kolm_smirn_param", "value"),
            dash.dependencies.Input("kolm_smirn_group_param", "value"),
        )(update_table)

        return html.Div(
            [
                html.Div(
                    html.H4(children="Критерий Колмогорова-Смирнова"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    dcc.Markdown(markdown_kolm_smirn_head),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(
                            [
                                ...
                                dcc.Dropdown(
                                    id="kolm_smirn_param",
                                    options=[
                                        {"label": i, "value": i} for i in cont_columns
                                    ],
                        ...
                        html.Div(
                            dcc.Markdown(markdown_student_variables_with_group),
                            style={"padding": "10px"},
                        ),
                    ]
                ),
                html.Div(
                    [
                        ...
                        html.Div(
                            dcc.Markdown(markdown_kolm_smirn), style={"padding": "20px"}
                        ),
                    ]
                ),
            ],
    ...
    def _generate_t_criterion_student_independent(self):
        ...
        self.app.callback(
            dash.dependencies.Output("table_t_criterion_student_ind", "data"),
            ...
            dash.dependencies.Input("student_ind_param", "value"),
        )(update_student_table)

        return html.Div(
            [
                html.Div(
                    html.H4(children="Т-критерий Стьюдента для независимых переменных"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    dcc.Markdown(markdown_student_ind_head),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            dcc.Markdown(markdown_student_p_value),
                            style={"padding": "20px"},
                        ),
                    ],
                    style={"padding": "20px"},
    ...
    def _generate_t_criterion_student_dependent(self):
        ...
        self.app.callback(
            dash.dependencies.Output("table_t_criterion_student_depend", "data"),
            ...
            dash.dependencies.Input("student_depend_param_2", "value"),
        )(update_student_table)

        return html.Div(
            [
                html.Div(
                    html.H4(children="Т-критерий Стьюдента для зависимых переменных"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    dcc.Markdown(markdown_student_depend_head),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        ...
                        html.Div(
                            dcc.Markdown(markdown_student_variables),
                            style={"padding": "10px"},
                        ),
                    ]
                ),
    ...
    def _generate_u_criterion_mann_whitney(self):
        ...
        def update_table_mann_whitney(group_var, var):
            ...
            df2.loc[1] = [
                ...
                str(data1.median())
                + " ("
                + str(np.percentile(data1, 25))
                + " - "
                + str(np.percentile(data1, 75))
                + ")",
                str(data2.median())
                + " ("
                + str(np.percentile(data2, 25))
                + " - "
                + str(np.percentile(data2, 75))
                + ")",
                p,
            ]
        ...
        self.app.callback(
            dash.dependencies.Output("table_u_criterion_mann_whitney", "data"),
            ...
            dash.dependencies.Input("whitney_param", "value"),
        )(update_table_mann_whitney)

        return html.Div(
            [
                html.Div(
                    html.H4(children="U-критерий Манна-Уитни"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    dcc.Markdown(markdown_mann_whitney_head),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
    ...
    def _generate_t_criterion_wilcoxon(self):
        ...
        self.app.callback(
            dash.dependencies.Output("table_t_criterion_wilcoxon", "data"),
            ...
            dash.dependencies.Input("wilcoxon_group_param_2", "value"),
        )(update_wilcoxon_table)

        return html.Div(
            [
                html.Div(
                    html.H4(children="T-критерий Уилкоксона"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    dcc.Markdown(markdown_wilcoxon_head), style={"text-align": "center"}
                ),
                html.Div(
                    [
                        html.Div(
    ...
    def _generate_chi2_pearson(self):
        ...
        self.app.callback(
            dash.dependencies.Output("conjugacy_table", "data"),
            dash.dependencies.Output("conjugacy_table", "columns"),
            dash.dependencies.Input("group_param_1_pearson", "value"),
            dash.dependencies.Input("group_param_2_pearson", "value"),
        )(update_conjugacy_table)
        ...
        def update_expected_table(var_1, var_2):
            ...
            metrics.loc[1] = [
                "alpha = 0.95",
                "X^2 = " + str(np.round(chi2_, 3)),
                np.round(chi2.isf(0.05, f_, loc=0, scale=1), 3),
                f_,
                p_,
            ]
            return (
                expected_df.to_dict("records"),
                [{"name": i, "id": i} for i in expected_df.columns],
                metrics.to_dict("records"),
                [{"name": i, "id": i} for i in metrics.columns],
            )

        self.app.callback(
            dash.dependencies.Output("expected_table", "data"),
            ...
            dash.dependencies.Input("group_param_1_pearson", "value"),
            dash.dependencies.Input("group_param_2_pearson", "value"),
        )(update_expected_table)

        return html.Div(
            [
                html.Div(
                    html.H4(children="Хи-квадрат Пирсона"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    dcc.Markdown(markdown_chi_2_head), style={"text-align": "center"}
                ),
                html.Div(
                    [
                        html.Div(
                ...
                html.Div(
                    [
                        html.Div(
                            html.H5(children="Таблица сопряженности"),
                            style={"text-align": "center"},
                        ),
                        html.Div(
                            dash_table.DataTable(
                ...
                html.Div(
                    [
                        html.Div(
                            html.H5(children="Ожидаемая частота"),
                            style={"text-align": "center"},
                        ),
                        html.Div(
                            dash_table.DataTable(
                ...
                html.Div(
                    [
                        html.Div(
                            html.H5(children="Таблица метрик"),
                            style={"text-align": "center"},
                        ),
                        html.Div(
                            dash_table.DataTable(
                        ...
                        html.Div(dcc.Markdown(markdown_chi_2)),
                    ],
                    style={"padding": "20px"},
                ),
            ],
    ...
    def _generate_sensitivity_specificity(self):
        ...
        def update_table(factor, result):
            ...
            table_se_sp = pd.DataFrame(
                columns=[
                    " ",
                    "Значение",
                    "Довирительный интервал Пирсона-Клоппера",
            ...
            table_se_sp.loc[1] = [
                "Чувствительность",
                np.round(se, 3),
                str(round(dov_in_se_clopper[0], 3))
                + "; "
                + str(round(dov_in_se_clopper[1], 3)),
                str(round(dov_in_se_wilson[0], 3))
                + "; "
            ...
            table_se_sp.loc[2] = [
                "Специфичность",
                np.round(sp, 3),
                str(round(dov_int_sp_clopper[0], 3))
                + "; "
                + str(round(dov_int_sp_clopper[1], 3)),
                str(round(dov_int_sp_wilson[0], 3))
                + "; "
            ...
            return (
                ...
                [{"name": i, "id": i} for i in table.columns],
                table_se_sp.to_dict("records"),
                [{"name": i, "id": i} for i in table_se_sp.columns],
            )

        self.app.callback(
            dash.dependencies.Output("pn_table", "data"),
            ...
            dash.dependencies.Input("result_se_sp", "value"),
        )(update_table)

        return html.Div(
            [
                html.Div(
                    html.H4(children="Чувствительность и Специфичность"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(
                            [
                                html.Div(
                                    [
                                        dcc.Markdown(children="Выберите фактор риска:"),
                                        dcc.Dropdown(
                                            id="factor_se_sp",
                                            options=[
                                                {"label": i, "value": i}
                                ...
                                html.Div(
                                    [
                                        dcc.Markdown(children="Выберите исход:"),
                                        dcc.Dropdown(
                                            id="result_se_sp",
                                            options=[
                                                {"label": i, "value": i}
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            dcc.Markdown(markdown_se_sep), style={"padding": "20px"}
                        ),
                    ],
                    style={"padding": "20px"},
                ),
    ...
    def _generate_odds_relations(self):
        ...
        def update_table(factor, result):
            ...
            return (
                table.to_dict("records"),
                [{"name": i, "id": i} for i in table.columns],
                table_res.to_dict("records"),
                [{"name": i, "id": i} for i in table_res.columns],
            )

        self.app.callback(
            dash.dependencies.Output("or_table", "data"),
            ...
            dash.dependencies.Input("result_odds", "value"),
        )(update_table)

        return html.Div(
            [
                html.Div(
                    html.H4(children="Отношение шансов"), style={"text-align": "center"}
                ),
                html.Div(
                    dcc.Markdown(markdown_odds_relations_head),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(
                            [
                                ...
                                html.Div(
                                    [
                                        dcc.Markdown(children="Выберите исход риска:"),
                                        dcc.Dropdown(
                                            id="result_odds",
                                            options=[
                                                {"label": i, "value": i}
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            dcc.Markdown(markdown_odds_relations),
                            style={"padding": "20px"},
                        ),
                    ],
                    style={"padding": "20px"},
    ...
    def _generate_risk_relations(self):
        ...
        def update_table(factor, result):
            ...
            ub = np.exp(
                np.log(rr)
                + 1.96 * np.sqrt(fn / (tp * (tp + fn)) + tn / (fp * (fp + tn)))
            )
            lb = np.exp(
                np.log(rr)
                - 1.96 * np.sqrt(fn / (tp * (tp + fn)) + tn / (fp * (fp + tn)))
        ...
        self.app.callback(
            dash.dependencies.Output("rr_table", "data"),
            ...
            dash.dependencies.Input("result_rr", "value"),
        )(update_table)

        return html.Div(
            [
                html.Div(
                    html.H4(children="Отношение рисков"), style={"text-align": "center"}
                ),
                html.Div(
                    dcc.Markdown(markdown_risk_relations_head),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            dcc.Markdown(markdown_risk_relations),
                            style={"padding": "20px"},
                        ),
                    ],
                    style={"padding": "20px"},

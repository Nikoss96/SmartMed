...
class LinearRegressionDashboard(Dashboard):
    ...
    def _generate_layout(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H1(children="Множественная регрессия"),
                    style={"text-align": "center"},
                ),
                dcc.Markdown(
                    children="Задачей множественной линейной регрессии является построение линейной модели связи между набором независимых переменных (предикторов) и зависимой переменной",
    ...
    def _generate_distrib(self):
        ...
        fig = go.Figure(data=go.Histogram(x=df_Y - predict_Y, name="Остатки"))

        fig.add_trace(
            go.Histogram(
                x=np.random.normal(0, 1, len(df_Y)), name="Нормальное распределение"
            )
        )
        ...
        fig_qqplot["layout"].update(
            xaxis={"title": "Теоретические квантили", "zeroline": True},
            yaxis={"title": "Экспериментальные квантили"},
            showlegend=True,
        )
        graph_styles = {
            "text-align": "center",
        ...
        return html.Div(
            [
                html.Div(
                    html.H2(children="Графики остатков"), style={"text-align": "center"}
                ),
                html.Div(
                    [
                        html.Div(
                        ...
                        html.Div(
                            dcc.Graph(id="Graph_ost_1", figure=fig), style=graph_styles
                        ),
                    ],
                    style={"margin": "50px"},
                ),
                html.Div(
                    [
                        html.Div(
                            html.H4(
                                children="График соответствия предсказанных значений зависимой переменной "
                                "и исходных значений"
                            ),
                            style={"text-align": "center"},
                        ),
                ...
                html.Div(
                    [
                        html.Div(
                            html.H4(children="График квантиль-квантиль"),
                            style={"text-align": "center"},
                        ),
                        html.Div(
                            dcc.Graph(id="graph_qqplot", figure=fig_qqplot),
                            style=graph_styles,
                        ),
                    ],
                    style={"margin": "50px"},
    ...
    def _generate_equation(self):
        ...
        def update_output(n_clicks, input1):
            ...
            if re.fullmatch(r"^([-+])?\d+([,.]\d+)?$", input1):
                ...
                if len(self.coord_list) == len(df_X.columns):
                    ...
                    return """Предсказанное значение равно {} \n Если желаете посчитать ещё для одного набор признаков
                    , напишите "Да".""".format(
                        round(yzn, 3)
                    )
            elif n_clicks > 0:
                return """Введено не число, введите значение параметра "{}" повторно.""".format(
                    df_X.columns[number]
                )
            if number == -1 and input1 != 0 and input1 != "Да" and input1 != "0":
                return """Если желаете посчитать ещё для {} набор признаков, напишите "Да".""".format(
                    "одного"
                )

        self.predict.app.callback(
        ...
        return html.Div(
            [
                html.Div(
                    html.H2(children="Уравнение множественной регрессии"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(dcc.Markdown(id="Markdown", children=uravnenie)),
                        html.Div(
                            html.H4(children="Предсказание новых значений"),
                            style={"text-align": "center"},
                        ),
                        dcc.Markdown(
                            children="Чтобы получить значение зависимой переменной, "
    ...
    def _generate_quality(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H2(children="Критерии качества модели"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(
                            dash_table.DataTable(
                                id="table1",
                                columns=[
                                    {"name": i, "id": i} for i in df_result_1.columns
                                ],
                                data=df_result_1.to_dict("records"),
                                export_format="xlsx",
                            ),
                            style={
                                "width": str(len(df_result_1.columns) * 8 - 10) + "%",
                                "display": "inline-block",
                            },
                        ),
    ...
    def _generate_resid(self):
        ...
        df_result_3 = pd.DataFrame(
            {
                "Номер наблюдения": 0,
                "Исходное значение признака": y,
                "Рассчитанное значение признака": znach,
                "Остатки": ost,
        ...
        return html.Div(
            [
                html.Div(
                    html.H2(children="Таблица остатков"), style={"text-align": "center"}
                ),
                html.Div(
                    [
                        html.Div(
                            ...
                            style={
                                "width": str(len(df_result_3.columns) * 8 - 10) + "%",
                                "display": "inline-block",
                            },
                        ),
    ...
    def _generate_signif(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H2(children="Критерии значимости переменных"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(
                            dash_table.DataTable(
                                id="table2",
                                columns=[
                                    {"name": i, "id": i} for i in df_result_2.columns
                                ],
                            ...
                            style={
                                "width": str(len(df_result_2.columns) * 6) + "%",
                                "display": "inline-block",
                            },
                        ),
                        html.Div(dcc.Markdown(markdown_linear_table2)),
                    ],  # style={'margin': '50px'},
                    style={
                        "width": "78%",
                        "display": "inline-block",
...
class LogisticRegressionDashboard(Dashboard):
    ...
    def _generate_layout(self):
        ...
        if np.any((df_X.data if issparse(df_X) else df_X) < 0):
            return html.Div(
                [
                    html.Div(
                        html.H1(children="Логистическая регрессия"),
                        style={"text-align": "center"},
                    ),
                    dcc.Markdown(
                        children=oprlog,
                    ...
                    html.Div(
                        html.H3(
                            children='Выбранная переменная - "{}"'.format(
                                self.predict.settings["y"]
                            ),
                            style={"text-align": "center"},
                        )
                    ),
                    html.Div(
                        dcc.Markdown(markdown_error),
                        style={
                            "width": "78%",
                            "display": "inline-block",
                            "border-color": "rgb(220, 220, 220)",
    ...
    def _generate_matrix(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H2(children="Матрица классификации"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(
                            dash_table.DataTable(
                                id="table_matrix",
                                columns=[
                                    {"name": i, "id": i} for i in df_matrix.columns
                                ],
                            ...
                            style={
                                "width": str(len(df_matrix.columns) * 8) + "%",
                                "display": "inline-block",
                            },
                        ),
                ...
                html.Div(
                    [
                        html.Div(
                            ...
                            style={
                                "width": str(len(df_result_1.columns) * 8) + "%",
                                "display": "inline-block",
                            },
                        ),
    ...
    def _generate_resid(self):
        df_matrix = pd.DataFrame(
            columns=["Accuracy", "Specificity", "Sensitivity", "AUC"]
        )
        y = self.predict.df_Y
        y.loc[y == y.min()] = -2
        y.loc[y == y.max()] = -1
        ...
        return html.Div(
            [
                html.Div(
                    html.H2(children="Прогностические меры"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
    ...
    def _generate_distrib(self):
        ...
        fig.add_shape(
            type="line", line=dict(dash="dash"), x0=-0.04, x1=1, y0=-0.04, y1=1
        )
        ...
        return html.Div(
            [
                html.Div(html.H2(children="Графики"), style={"text-align": "center"}),
                html.Div(
                    [
                        html.Div(
                        ...
                        html.Div(
                            [
                                dcc.Graph(id="Graph_ost_1", figure=fig),
    ...
    def _generate_signif(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H2(children="Критерии  переменных"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(
                            ...
                            style={
                                "width": str(len(df_result_2.columns) * 10) + "%",
                                "display": "inline-block",
                            },
                        ),
                        html.Div(dcc.Markdown(crit), style={"padding": "20px"}),
                    ],  # style={'margin': '50px'},
                    style={
                        "width": "78%",
                        "display": "inline-block",
...
class PolynomRegressionDashboard(Dashboard):
    ...
    def _generate_layout(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H1(children="Полиномиальная регрессия"),
                    style={"text-align": "center"},
                ),
                html.Div(metrics_list),
            ]
    ...
    def _generate_equation(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H2(children="Уравнение полииномиальной регрессии"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
...
class ROC(Dashboard):
    ...
    def _generate_layout(self):
        if "classificators_comparison" in self.predict.settings["metrics"]:
            metrics_list = [self._generate_dashboard(), self._generate_comparison()]
        else:
        ...
        return html.Div(
            [
                html.Div(
                    html.H1(children="ROC-анализ"), style={"text-align": "center"}
                ),
                html.Div(metrics_list),
            ]
        )
    ...
    def _generate_metrics(self, ind):
        ...
        TPR = round(
            self.tp_list[ind][t_ind]
            / (self.tp_list[ind][t_ind] + self.fn_list[ind][t_ind]),
            3,
        )
        PPV = round(
            self.tp_list[ind][t_ind]
            / (self.tp_list[ind][t_ind] + self.fp_list[ind][t_ind]),
            3,
        )
        specificity = round(
            self.tn_list[ind][t_ind]
            / (self.tn_list[ind][t_ind] + self.fp_list[ind][t_ind]),
            3,
        )
        accuracy = round(
            (self.tp_list[ind][t_ind] + self.tn_list[ind][t_ind])
            / (
                self.tp_list[ind][t_ind]
                + self.fn_list[ind][t_ind]
                + self.tn_list[ind][t_ind]
        ...
        for i in range(len(self.sp_list[ind]) - 1):
            auc += (
                (self.se_list[ind][i] + self.se_list[ind][i + 1])
                * (self.inv_sp_list[ind][i + 1] - self.inv_sp_list[ind][i])
                / 2
            )
        auc = round(abs(auc), 3)
        ...
        df_ost_2.loc[1] = [
            "Значение",
            threshold,
            round(self.dx_list[ind][t_ind], 3),
            TPR,
            specificity,
            PPV,
            accuracy,
        ...
        # dov int
        # Se
        di_se_clopper = dov_int_clopper(
            self.tp_list[ind][t_ind],
            self.tp_list[ind][t_ind] + self.fn_list[ind][t_ind],
        )
        di_se_wilson = dov_int_wilson(
            self.tp_list[ind][t_ind],
            self.tp_list[ind][t_ind] + self.fn_list[ind][t_ind],
        )
        # Sp
        di_sp_clopper = dov_int_clopper(
            self.tn_list[ind][t_ind],
            self.tn_list[ind][t_ind] + self.fp_list[ind][t_ind],
        )
        di_sp_wilson = dov_int_wilson(
            self.tn_list[ind][t_ind],
            self.tn_list[ind][t_ind] + self.fp_list[ind][t_ind],
        )
        # Precision
        di_prec_clopper = dov_int_clopper(
            self.tp_list[ind][t_ind],
            self.tp_list[ind][t_ind] + self.fp_list[ind][t_ind],
        )
        di_prec_wilson = dov_int_wilson(
            self.tp_list[ind][t_ind],
            self.tp_list[ind][t_ind] + self.fp_list[ind][t_ind],
        )
        # Accuracy
        di_accur_clopper = dov_int_clopper(
            self.tp_list[ind][t_ind] + self.tn_list[ind][t_ind],
            self.tp_list[ind][t_ind]
            + self.fn_list[ind][t_ind]
            + self.tn_list[ind][t_ind]
        ...
        di_accur_wilson = dov_int_wilson(
            self.tp_list[ind][t_ind] + self.tn_list[ind][t_ind],
            self.tp_list[ind][t_ind]
            + self.fn_list[ind][t_ind]
            + self.tn_list[ind][t_ind]
        ...
        # AUC
        di_auc = (
            np.var(self.se_list[ind])
            / (len(self.se_list[ind]) * (len(self.se_list[ind]) - 1))
        ) ** 0.5
        ...
        df_dov_int.loc[1] = [
            ...
            str(round(di_prec_clopper[0], 3))
            + "; "
            + str(round(di_prec_clopper[1], 3)),
            str(round(di_accur_clopper[0], 3))
            + "; "
            + str(round(di_accur_clopper[1], 3)),
            str(di_auc_1) + "; " + str(di_auc_2),
        ]
        df_dov_int.loc[2] = [
            "Вилсона",
            str(round(di_se_wilson[0], 3)) + "; " + str(round(di_se_wilson[1], 3)),
            str(round(di_sp_wilson[0], 3)) + "; " + str(round(di_sp_wilson[1], 3)),
            str(round(di_prec_wilson[0], 3)) + "; " + str(round(di_prec_wilson[1], 3)),
            str(round(di_accur_wilson[0], 3))
            + "; "
            + str(round(di_accur_wilson[1], 3)),
            str(di_auc_1) + "; " + str(di_auc_2),
        ]
    ...
    def _generate_dots(self, ind):
        df_ost_2 = pd.DataFrame(
            {
                "dx": [
                    round(self.dx_list[ind][i], 3)
                    for i in range(len(self.dx_list[ind]))
                ],
                "tp": self.tp_list[ind],
                "fp": self.fp_list[ind],
                "fn": self.fn_list[ind],
    ...
    def _generate_interception(self, ind):
        ...
        fig_rasp_2.update_xaxes(
            title_text="Порог отсечения", title_font={"size": 20}, title_standoff=25
        )
        fig_rasp_2.update_yaxes(
            title_text="Значение", title_font={"size": 20}, title_standoff=25
        )
        fig_rasp_2.update_traces(marker_size=10)
    ...
    def _generate_dashboard(self):
        ...
        for i in range(len(columns_list)):
            ...
            while True:
                ...
                if (len(tp_list) == 0) or (
                    len(tp_list) > 0
                    and (TP != tp_list[-1] or TN != tn_list[-1] or FP != fp_list[-1])
                ):
        ...
        def update_roc(column_name, self=self):
            ...
            dov_list_1 = [
                self.se_list[ind][i] - 1.96 * dov_int
                for i in range(len(self.se_list[ind]))
            ]
            dov_list_2 = [
                self.se_list[ind][i] + 1.96 * dov_int
            ...
            fig_roc.update_xaxes(
                title_text="1-Специфичность", title_font={"size": 20}, title_standoff=25
            )
        ...
        self.predict.app.callback(
            dash.dependencies.Output("graph_roc", "figure"),
            dash.dependencies.Input("metric_name", "value"),
        )(update_roc)
        ...
        def update_inter(column_name, self=self):
            ...
            fig_inter.update_yaxes(
                title_text="Значения", title_font={"size": 20}, title_standoff=25
            )
            fig_inter.update_traces(marker_size=10)
            return fig_inter

        self.predict.app.callback(
            dash.dependencies.Output("graph_inter", "figure"),
            dash.dependencies.Input("metric_name", "value"),
        )(update_inter)
        ...
        self.predict.app.callback(
            dash.dependencies.Output("table_dot", "data"),
            dash.dependencies.Input("metric_name", "value"),
        )(update_table_dot)
        ...
        self.predict.app.callback(
            dash.dependencies.Output("table_inter", "data"),
            dash.dependencies.Input("metric_name", "value"),
        )(update_table_inter)
        ...
        self.predict.app.callback(
            dash.dependencies.Output("table_metrics", "data"),
            dash.dependencies.Input("metric_name", "value"),
        )(update_metrics)
        ...
        self.predict.app.callback(
            dash.dependencies.Output("table_dov_int_1", "data"),
            dash.dependencies.Input("metric_name", "value"),
        )(update_dov_int)
        ...
        div_roc = html.Div(
            [
                ...
                html.Div(
                    [
                        html.Div(
                            dcc.Graph(id="graph_roc", figure=fig_roc),
                            style={
                                "text-align": "center",
                                "width": "78%",
                                "display": "inline-block",
        ...
        div_metrics = html.Div(
            [
                html.Div(
                    html.H4(children="Таблица метрик"), style={"text-align": "center"}
                ),
                html.Div(
                    [
                        html.Div(
                            dash_table.DataTable(
                                id="table_metrics",
                                columns=[
                                    {"name": i, "id": i} for i in df_metrics.columns
                                ],
                            ...
                            style={
                                ...
                                "border-style": "solid",
                                "text-align": "center",
                                "width": str(len(df_metrics.columns) * 10 - 10) + "%",
                                "display": "inline-block",
                            },
                        ),
        ...
        div_dov_int = html.Div(
            [
                html.Div(
                    html.H4(children="Таблица доверительных интервалов"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(
                            dash_table.DataTable(
                                id="table_dov_int_1",
                                columns=[
                                    {"name": i, "id": i} for i in df_dov_int.columns
                                ],
                            ...
                            style={
                                ...
                                "border-style": "solid",
                                "text-align": "center",
                                "width": str(len(df_dov_int.columns) * 10 - 10) + "%",
                                "display": "inline-block",
                            },
                        ),
        ...
        div_dot = html.Div(
            [
                html.Div(
                    html.H4(children="Таблица точек ROC"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(
                            ...
                            style={
                                ...
                                "border-style": "solid",
                                "text-align": "center",
                                "width": str(len(df_dots.columns) * 10 - 10) + "%",
                                "display": "inline-block",
                            },
                        ),
        ...
        div_inter = html.Div(
            [
                html.Div(
                    html.H4(children="График пересечения"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                        html.Div(
                            dcc.Graph(id="graph_inter", figure=fig_inter),
                            style={
                                "text-align": "center",
                                "border-color": "rgb(220, 220, 220)",
                                "border-style": "solid",
    ...
    def _generate_comparison(self):
        ...
        self.predict.app.callback(
            dash.dependencies.Output("graph_roc_2", "figure"),
            [
                dash.dependencies.Input("group_param_1", "value"),
                dash.dependencies.Input("group_param_2", "value"),
            ],
        )(update_roc_2)

        div_2_title = html.Div(
            html.H2(children="Блок сравнения"), style={"text-align": "center"}
        )
        ...
        div_2_roc = html.Div(
            [
                ...
                html.Div(
                    [
                        html.Div(
                            dcc.Graph(id="graph_roc_2", figure=fig_roc_2),
                            style={
                                "text-align": "center",
                                "width": "78%",
                                "display": "inline-block",
                    ...
                    [
                        html.Div(
                            dash_table.DataTable(
                                id="table_metrics_2",
                                columns=[
                                    {"name": i, "id": i} for i in sum_table.columns
                                ],
                            ...
                            style={
                                ...
                                "border-style": "solid",
                                "text-align": "center",
                                "width": str(len(sum_table.columns) * 10 - 10) + "%",
                                "display": "inline-block",
                            },
                        ),
                    ...
                    [
                        html.Div(
                            dash_table.DataTable(
                                id="table_dov_int_2",
                                columns=[
                                    {"name": i, "id": i} for i in sum_table_di.columns
                                ],
                            ...
                            style={
                                ...
                                "border-style": "solid",
                                "text-align": "center",
                                "width": str(len(sum_table_di.columns) * 10 - 10) + "%",
                                "display": "inline-block",
                            },
                        ),
                        html.Div(dcc.Markdown(" ")),
...
class TreeDashboard(Dashboard):
    ...
    def _generate_layout(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H1(children="Дерево классификации"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    html.H5(children=markdown_introduction),
                    style={"text-align": "center"},
                ),
                html.Div(
                    html.H3(
    ...
    def _generate_tree_graph(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H3(children="Графическое представление дерева"),
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
    ...
    def _generate_table(self):
        ...
        df = pd.DataFrame(
            {"Наблюдаемые показатели": df_Y_new, "Предсказание": predict_Y_new}
        )
        ...
        return html.Div(
            [
                html.Div(
                    [
                        html.Div(
                            html.H3(children="Классификационная таблица"),
                            style={"text-align": "center"},
                        ),
                        html.Div(
                            [
    ...
    def _generate_indicators(self):
        ...
        df = pd.DataFrame(
            {
                "Score": accuracy,
                "Энтропийный индекс неоднородности": entropy_heterogeneity,
                "Индекс Джини": gini,
                "Индекс ошибочной классификации": index_wrong_classification,
            },
            index=[0],
        )
        # Table if 2 classes
            ...
            [
                html.Div(
                    [
                        html.Div(
                            html.H3(
                                children="Показатели качества построенного дерева решений"
                            ),
                            style={"text-align": "center"},
                        ),
                        html.Div(
    ...
    def _generate_class_distributions(self):
        ...
        self.predict.app.callback(
            dash.dependencies.Output("graph_distributions", "figure"),
            dash.dependencies.Input("x_name", "value"),
            dash.dependencies.Input("y_name", "value"),
        )(update_graph)

        return html.Div(
            [
                html.H3(
                    children="График распределения классов",
                    style={"text-align": "center"},
                ),
                html.Div(
                    [
                ...
                html.Div(
                    [dcc.Graph(id="graph_distributions")],
                    style={"width": "100%", "display": "inline-block"},
                ),
            ],
            style={
    ...
    def _generate_prediction_block(self):
        ...
        self.predict.app.callback(
            dash.dependencies.Output("predict_results", "data"),
            dash.dependencies.Input("predict_table", "data"),
            dash.dependencies.Input("btn_ok", "n_clicks"),
        )(get_data)
        ...
        return html.Div(
            [
                html.Div(
                    html.H3(
                        children="Работа с исходной таблицей для получения предсказательных значений"
                    ),
                    style={"text-align": "center"},
                ),
                dcc.Markdown(
                    children="Вы можете изменить исходные данные и оценить предсказанное значение"
                ),
                html.Div(
                    [
                        dash_table.DataTable(

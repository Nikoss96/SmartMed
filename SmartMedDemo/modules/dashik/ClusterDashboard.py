...
class ClusterDashboard(Dashboard):
    def _generate_layout(self):
        ...
        return html.Div(
            [
                html.Div(
                    html.H1(children="Кластерный анализ"),
                    style={"text-align": "center"},
                ),
                dcc.Markdown(
                    children=markdown_clus,
    ...
    def _generate_kmeans(self):
        ...
        def k_znach(n, met):
            try:
                    ...
                    if metric == 0:
                        ...
                        kmeans_instance = kmeans(
                            ...
                            metric=distance_metric(
                                type_metric.USER_DEFINED, func=user1
                            ),
                        )
                    elif metric == 5:
                        kmeans_instance = kmeans(
                            ...
                            initial_centers,
                            metric=distance_metric(
                                type_metric.USER_DEFINED, func=user2
                            ),
                        )
                ...
                d = {
                    "Кластер": pd.Series(clusters2, index=[i for i in range(n)]),
                    "Число элементов в кластере": pd.Series(
                        num, index=[i for i in range(n)]
                    ),
                    "Начальные центры кластера": pd.Series(
                        beg2, index=[i for i in range(n)]
                    ),
                    "Конечные центры кластера": pd.Series(
                        fin2, index=[i for i in range(n)]
                    ),
                }
                df1 = pd.DataFrame(d)
                print(df1)
        ...
        def update_output_div(n, met):
            ...
            return html.Div(
                [
                    html.Div(
                        html.H2(children="Исходные и конечные центры кластеров"),
                        style={"text-align": "center"},
                    ),
                        ...
                        [
                            html.Div(
                                dash_table.DataTable(
                                    columns=[
                                        {"name": i, "id": i} for i in cent.columns
                                    ],
                    ...
                    html.Div(
                        html.H2(children="Принадлежность элементов кластерам"),
                        style={"text-align": "center"},
                    ),
                    ...
                    html.Div(
                        [
                            html.Div(
                                dash_table.DataTable(
                                    columns=[
                                        {"name": i, "id": i} for i in clust.columns
                                    ],
        ...
        self.app.callback(
            Output("tab1", "children"), [Input("n", "value"), Input("met", "value")]
        )(update_output_div)
        ...
        return html.Div(
            [
                html.Div(
                    [
                        html.Div(
                            html.H1(children="Кластеризация k-средними"),
                            style={"text-align": "center"},
                        ),
    ...
    def _generate_hirarchy(self):
        ...
        def update_output_div(n, met, meth):
            ...
            model = AgglomerativeClustering(
                n_clusters=n, affinity=met, linkage=meth
            ).fit(x)
            labs = model.labels_
            lst = []
            ...
            di = sorted(
                linkage(x, method=meth, metric=met, optimal_ordering=False)[:, 2]
            )

            fig = ff.create_dendrogram(
                df,
                linkagefun=lambda ci: linkage(df, meth, metric=met),
                color_threshold=di[-n + 1],
            )
            fig.update_layout(autosize=True, hovermode="closest")
            fig.update_xaxes(
                mirror=False, showgrid=True, showline=False, showticklabels=False
            )
        ...
        return html.Div(
            [
                html.Div(
                    [
                        html.Div(
                            html.H1(children="Иерархическая кластеризация"),
                            style={"text-align": "center"},
                        ),
    ...
    def _generate_component(self):
        ...
        def update_output_div(m, metr):
                ...
                if len(i) != 1:
                    if lst:
                        ...
                        for j in range(len(lst)):
                            if j - 1 < len(lst):
                                if list(
                                    set(lst[j - 1]) & set(i) or set(lst[j - 1]) & set(t)
                                ):
                                    if t == []:
            ...
            for i in lst:
                num.append(len(i))
            d = {
                "Кластер": pd.Series(clusters2, index=[i for i in range(len(lst))]),
                "Число элементов в кластере": pd.Series(
                    num, index=[i for i in range(len(lst))]
                ),
            }
            df1 = pd.DataFrame(d)
            return (
        ...
        self.app.callback(
            [
                Output("my-1", "children"),
                Output("fig1", "figure"),
                Output("fig2", "figure"),
            ],
            [Input("m", "value"), Input("metr", "value")],
        )(update_output_div)

        return html.Div(
            [
                html.Div(
                    html.H1(children="Кластеризация выделением связных компонент"),
                    style={"text-align": "center"},
                ),
                ...
                html.Div(
                    [
                        html.Div(
                            html.H2(children="Гистограмма попарных расстояний"),
                            style={"text-align": "center"},
                        ),
                        ...
                        html.Div(
                            html.H2(children="Представление выборки в виде графа"),
                            style={"text-align": "center"},
                        ),

...
def display_scatter(dataframe: pd.DataFrame, axis_x: str, axis_y: str,
...
def display_correlation_matrix(dataframe: pd.DataFrame, sharey=False,
                               annot=True, Pearson=True, Spearman=True,
                               title="",
    ...
    f, axes = plt.subplots(nrows=1, ncols=ncols, sharey=sharey,
    ...
    if Pearson:
        if (Spearman):
            pltP = sns.heatmap(dataframe.corr(method='pearson'), annot=annot,
                               vmin=-1, vmax=1, cmap=cmap, center=0, fmt=fmt,
                               ax=axes[0])
        else:
            pltP = sns.heatmap(dataframe.corr(method='pearson'), annot=annot,
                               vmin=-1, vmax=1, cmap=cmap, center=0, fmt=fmt)
    ...
    if Spearman:
        if (Pearson):
            pltS = sns.heatmap(dataframe.corr(method='spearman'), annot=annot,
                               vmin=-1, vmax=1, cmap=cmap, center=0, fmt=fmt,
                               ax=axes[1])
        else:
            pltS = sns.heatmap(dataframe.corr(method='spearman'), annot=annot,
...
def sheet_to_dataframe_spec(sheet, start_letter: str, start_row: str,
                            finish_letter: str, finish_row: str):
    x = []

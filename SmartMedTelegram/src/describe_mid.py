...
def display_scatter(dataframe: pd.DataFrame, axis_x: str, axis_y: str, title_x = None, title_y = None):
    if not title_x:
        title_x = axis_x
    if not title_y:
        title_y = axis_y
...
def display_correlation_matrix(dataframe: pd.DataFrame, sharey = False, annot=True, Pearson = True, Spearman = True, title = "", cmap = sns.color_palette("viridis", as_cmap=True), fmt = ".2f"):
    ...
    f, axes = plt.subplots(nrows = 1, ncols = ncols, sharey=sharey, figsize = (FIG_WIDTH * ncols, FIG_HEIGHT))

    pltP = None
    pltS = None
    if Pearson:
        if (Spearman):
            pltP = sns.heatmap(dataframe.corr(method = 'pearson'), annot = annot, vmin=-1, vmax=1, cmap = cmap, center = 0, fmt = fmt, ax = axes[0])
        else:
            pltP = sns.heatmap(dataframe.corr(method = 'pearson'), annot = annot, vmin=-1, vmax=1, cmap = cmap, center = 0, fmt = fmt)
        pltP.xaxis.tick_top()
        pltP.set_title("Пирсон. " + title)
        pltP.set_xticklabels(pltP.get_xticklabels(), rotation = 30)
    if Spearman:
        if (Pearson):
            pltS = sns.heatmap(dataframe.corr(method = 'spearman'), annot = annot, vmin=-1, vmax=1, cmap = cmap, center = 0, fmt = fmt, ax = axes[1])
        else:
            pltS = sns.heatmap(dataframe.corr(method = 'spearman'), annot = annot, vmin=-1, vmax=1, cmap = cmap, center = 0, fmt = fmt)
        pltS.xaxis.tick_top()

...
# Словарь терминов
dict = {
    "T-критерий Стьюдента для независимых переменных": "статистический критерий, применяемый для сравнения средних значений двух независимых выборок. Он основан на t-распределении и используется для определения, есть ли статистически значимые различия между средними значениями двух групп.",
...
keyboard_dict.add(
    telebot.types.InlineKeyboardButton(
        text="Кривая выживаемости", callback_data="curve"
    )
)
keyboard_dict.add(
    telebot.types.InlineKeyboardButton(
        text="Диаграмма Ящик с усами", callback_data="box"
    )
)
keyboard_dict.add(
...
def callback_query(call):
    if call.data == "example_bioequal":
        bot.answer_callback_query(
            call.id, "Прислали вам пример файла. Оформляйте в точности так."
        )
        ...
        bot.send_message(
            call.from_user.id,
            "Вы снова можете выбрать модуль.",
...
def default_test(message):
    if message.text.lower() == "bioequal":
        ...
        keyboard00.add(
            telebot.types.InlineKeyboardButton(
                text="Пример файла", callback_data="example_bioequal"
            )
        )
        keyboard00.add(
            telebot.types.InlineKeyboardButton(
        ...
        bot.send_message(
            message.chat.id,
            "Готовы загрузить сразу или требуется пояснение?",
            reply_markup=keyboard00,
        )
    elif message.text.lower() == "describe":
        keyboard01 = telebot.types.InlineKeyboardMarkup()
        keyboard01.add(
            telebot.types.InlineKeyboardButton(
                text="Пример файла", callback_data="example_describe"
            )
        )
        keyboard01.add(
            telebot.types.InlineKeyboardButton(
                text="Загружу свой", callback_data="download_describe"
            )
        )
        bot.send_message(
            message.chat.id,
            "Готовы загрузить сразу или требуется пояснение?",
            reply_markup=keyboard01,
        )
    elif message.text.lower() == "predict":
        keyboard02 = telebot.types.InlineKeyboardMarkup()
        keyboard02.add(
            telebot.types.InlineKeyboardButton(
                text="Пример файла", callback_data="example_predict"
            )
        )
        keyboard02.add(
            telebot.types.InlineKeyboardButton(
        ...
        bot.send_message(
            message.chat.id,
            "Выберите модуль из предложенных ниже.",
        ...
        bot.send_message(
            message.chat.id,
            "Выберите интересующий вас термин:",
